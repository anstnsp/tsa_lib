...
import com.anstn.test.response.CommonResult;
import com.anstn.test.util.HttpConnectionConfig;
import com.anstn.test.util.Util;
...
public class SigningService {
	    ...
	    public byte[] signPdf(byte[] pdfToSign) throws NullPointerException, GeneralSecurityException, IOException {
	        try {
	        	...
	        	PrivateKey privateKey = Util.getPrivateKey(this.privKeyPath);     //NoSuchAlgorithmException InvalidKeySpecException  NullPointerException IOException
	            X509Certificate certificate = Util.getPublicKey(this.pubKeyPath);
	            Signature signature = new Signature(privateKey, certificate, this.tsaUrl);   //UnrecoverableKeyException CertificateNotYetValidException CertificateExpiredException //KeyStoreException //NoSuchAlgorithmException //IOException
		...
		public boolean toBlcokChain(byte[] fileContent, byte[] signedPdf, String makeFileFullPath) throws OperatorCreationException, CMSException, CertificateException, IOException, NoSuchAlgorithmException, ConException, TSPException, NoTimeStampException, DAppException {
		 	PDDocument doc = null;
			...
			try {
	    		...
	    		HttpResponse httpRes = connection.doPost(this.tsaUrl + "/" + "insertblc", param);
